...
if [[ -z "$1" ]]; then
  echo "No selection mode specified, Going with auto."
  automatic_selection
elif [[ "$1" == "auto" || "$1" == "automatic" ]]; then
  echo "Selection mode: automatic"
  automatic_selection
elif [[ "$1" == "man" || "$1" == "manual" ]]; then
  echo "Selection mode: automatic"
  manual_selection
else 
  echo "USAGE: keyboard-backlight <SELECTION MODE>"
  echo "Available modes: auto|automatic, man|manual"
  echo "INVALID/NULL selection mode value"
